    ...
    private static File getAppt(Integer version) throws BrutException {
        ...
        try {
            if (OSDetection.isArm()) {
                aaptBinary = new File(Apktool.getHomeDir(), "/prebuilt/" + aaptVersion + "/armeabi-v7a/" + aaptVersion);
            } else if (OSDetection.isArm64()) {
                aaptBinary = new File(Apktool.getHomeDir(), "/prebuilt/" + aaptVersion + "/arm64-v8a/" + aaptVersion);
            } else {
